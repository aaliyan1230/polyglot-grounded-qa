...
def test_materialize_final_results_and_contract_core() -> None:
    ...
    assert (root / "artifacts/tables/final_hybrid_abstention_summary.parquet").exists()
    ...
    abstention_summary = pl.read_parquet(root / "artifacts/tables/final_hybrid_abstention_summary.parquet")
    assert {
        "variant",
        "abstain_accuracy",
        "abstain_precision",
        "abstain_recall",
    }.issubset(set(abstention_summary.columns))
